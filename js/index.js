...
// -----------------MOUSE CLICK-----------------
    ...
    for(let key of keys){
        addEventListener("mousedown", e => {
            polySynth.triggerAttackRelease(elementToPitch[e.key])
        })
...
function kick(){
    ...
    kick.chain(gain, filter, freeverb, Tone.Master)
    var loop = new Tone.Loop(function(time){
        kick.triggerAttackRelease("C2", "4n");
    }, "4n").start(0);
    Tone.Transport.start();
    
    let kickbutton = document.querySelector(".rythm")
...
function snare(){
    ...
    var gain = new Tone.Gain(0.1)
    var filter = new Tone.Filter(5000, "lowpass", -12)
    var freeverb = new Tone.Freeverb()
    freeverb.dampening.value = 1000;
    freeverb.roomSize.value = 0.5;
    freeverb.wet.value = 0.5
    var snare = new Tone.MembraneSynth()
    snare.chain(gain, filter, freeverb, Tone.Master)
    var loop = new Tone.Loop(function(time){
        snare.triggerAttackRelease("C3", "4n");
    }, "2n").start(0);
    Tone.Transport.start();
